...
Select "Next", then "Launch Scan" to validate the connection and start the first OCI scan. Prowler discovers subscribed regions from OCI, and the private key content is encoded for secure transmission.
...
- Example policy:
  ```
  Allow dynamic-group prowler-instances to inspect tenancies in tenancy
...
```
Allow group prowler-users to inspect tenancies in tenancy
...
The `inspect tenancies` permission grants the OCI `TENANCY_INSPECT` permission required for `ListRegionSubscriptions`, which Prowler uses to discover the subscribed OCI regions before scanning. Prowler cannot start an OCI scan if it cannot retrieve the subscribed regions.
...
**Error: "Could not retrieve subscribed OCI regions"**
- Ensure the principal has `Allow group prowler-users to inspect tenancies in tenancy` or the equivalent dynamic-group policy
- Prowler uses `ListRegionSubscriptions`, which requires the OCI `TENANCY_INSPECT` permission, to discover subscribed regions before scanning.
